...
>
  <div class="md:p-2 lg:items-start lg:py-2 lg:pr-8 flex flex-col lg:w-[100%]">
    ...
    <div class="grid grid-flow-col overflow-x-scroll auto-cols-max gap-4 lg:gap-6 no-scrollbar lg:w-[100%] ">
    ...
    {:else if item?.tabVideoOrImage.includes("/image")}
      ...
      />
    {:else}
        <div class="mt-6 relative w-[100%] overflow-hidden pt-[56.25%] rounded-lg">
      <iframe
        width="100%"
        src={item?.tabVideoOrImage}
        frameborder="0"
    ...
    <div class="flex mt-6 lg:justify-start mb-6 lg:mt-8">
      ...
      >
        <a href={item?.tabButtonLink} class="flex gap-1 items-center" data-testid="link">
          {item?.tabButtonText}
          <svg
            fill="#ffffff"
            height="12px"
